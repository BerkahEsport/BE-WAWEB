...
module.exports = async(conn, m) => {
    ...
    let header_client = chalk.red("~ " + conn.info.pushname + " " + PhoneNumber("+" + conn.info.wid.user).getNumber("international")) + " " + chalk.black(chalk.bgGreenBright(conn.info.platform.replace(/^./, (v) => v.toUpperCase()))) + " " + chalk.black(chalk.bgYellow((m.timestamp ? new Date(1000 * (m.timestamp.low || m.timestamp)) : new Date).toTimeString()))
